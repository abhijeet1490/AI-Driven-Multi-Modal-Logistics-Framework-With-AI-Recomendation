...
multi-output regression model, and saves it to `backend/models/logistics_model.pkl`.
"""
...
def generate_synthetic_data(
    ...
    cost_usd += rng.normal(0, 50.0, size=config.n_samples)
    ...
    time_hours += rng.normal(0, 0.3, size=config.n_samples)
    time_hours = np.clip(time_hours, 0.1, None)

    co2_per_km = 0.18
    co2_per_kg = 0.0004
    co2_traffic_factor = 1.5
    ...
    co2_kg += rng.normal(0, 10.0, size=config.n_samples)
...
def main() -> None:
    ...
    # Calculate model path into backend/models directory
    base_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(base_dir, "..", "backend", "models", "logistics_model.pkl")

    print(f"Saving model to '{os.path.relpath(model_path)}'...")
